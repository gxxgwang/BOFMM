...
def make_animation(source_image, driving_video,generator, kp_detector, relative=True, adapt_movement_scale=True, cpu=False):
    with torch.no_grad():
        ...
        driving = torch.tensor(np.array(driving_video)[np.newaxis].astype(np.float32)).permute(0, 3, 1, 2)
        kp_source = kp_detector(source)
        # kp_driving_initial = kp_detector(driving[:, :, 0].cuda())
        kp_driving_initial = kp_detector(driving.cuda())
        ...
        out = generator(source, driving_frame, kp_source=kp_source, kp_driving=kp_norm)
...
if __name__ == "__main__":
    ...
    driving_video = resize(driving_video, (256, 256))[..., :3]
    ...
    for frame in predictions:
        ...
        i = i + 1
    # imageio.imsave(opt.result_video, [img_as_ubyte(frame) for frame in predictions])
