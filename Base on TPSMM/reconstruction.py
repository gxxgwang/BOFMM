        ...
        with torch.no_grad():
            ...
            for frame_idx in range(x['video'].shape[2]):
                ...
                if bg_predictor:
                    ...
                    bg_params2 = bg_predictor(driving, source)
                
                dense_motion = dense_motion_network(source_image=source, kp_driving=kp_driving,
                                                    kp_source=kp_source, bg_param = bg_params, 
                                                    dropout_flag = False)
                dense_motion2 = dense_motion_network(source_image=driving, kp_driving=kp_source,
                                                    kp_source=kp_driving, bg_param = bg_params2, 
                                                    dropout_flag = False)
                ...
                loss_list.append(torch.abs(out['prediction_sd'] - driving).mean().cpu().numpy())
