...
def make_animation(source_image, driving_video, inpainting_network, kp_detector, dense_motion_network, avd_network,
    ...
    with torch.no_grad():
        ...
        driving = torch.tensor(np.array(driving_video)[np.newaxis].astype(np.float32)).permute(0, 3, 1, 2).to(device)
        ...
        if mode == 'standard':
            ...
            kp_norm2 = relative_kp(kp_source=kp_driving, kp_driving=kp_source,
                                   kp_driving_initial=kp_source)
        elif mode == 'avd':
            kp_norm = avd_network(kp_source, kp_driving)
            kp_norm2 = avd_network(kp_driving, kp_source)
        dense_motion = dense_motion_network(source_image=source, kp_driving=kp_norm,
                                            kp_source=kp_source, bg_param=None,
                                            dropout_flag=False)
        dense_motion2 = dense_motion_network(source_image=driving, kp_driving=kp_norm2,
                                             kp_source=kp_driving, bg_param=None,
                                             dropout_flag=False)
        out = inpainting_network(source, driving, dense_motion, dense_motion2)

        predictions.append(np.transpose(out['prediction_sd'].data.cpu().numpy(), [0, 2, 3, 1])[0])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default='config/mgif-256.yaml', help="path to config")
    parser.add_argument("--checkpoint", default='mgif.pth.tar', help="path to checkpoint to restore")

    parser.add_argument("--source_image", default='C:\\Users\\Desktop\\fashion\\test\\S.png',
                        help="path to source image")
    parser.add_argument("--driving_video", default='C:\\Users\\Desktop\\fashion\\test\\D2.png',
                        help="path to driving video")
    parser.add_argument("--result_video", default='C:\\Users\\Desktop\\fashion\\result_tps121',
                        help="path to output")
    ...
    parser.add_argument("--mode", default='standard', choices=['standard', 'relative', 'avd'],
    ...
    driving_video = imageio.imread(opt.driving_video)
    ...
    driving_video = resize(driving_video, opt.img_shape)[..., :3]
    ...
    for frame in predictions:
        imageio.imsave(os.path.join(opt.result_video, "{:05d}.png".format(i)), img_as_ubyte(frame))
        i = i + 1
